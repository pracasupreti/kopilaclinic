...
export default function Header() {
  ...
  return (
    <header className="bg-green-50 shadow-md border-b border-gray-300 px-4 md:px-12 py-5 top-0 z-50"> {/* sticky */}
      <nav className="max-w-screen-xl mx-auto flex items-center justify-between relative">
        ...
        <div
          ...
          } fixed md:static top-0 left-0 h-full w-4/5 md:w-auto md:h-auto md:translate-x-0 transform transition-transform duration-300 ease-in-out bg-white md:bg-transparent z-40 flex flex-col md:flex-row md:items-center md:space-x-10 px-6 pt-24 pb-6 md:p-0 shadow-lg md:shadow-none`}
        >
          <ul className="flex flex-col md:flex-row gap-y-6 md:gap-y-0 md:gap-x-10 text-sm md:text-base font-semibold text-primary">
            {NAV_PAGES.map((page) => (
              <li key={page.name}>
                <Link
                  href={page.path}
